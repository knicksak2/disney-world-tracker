// Feature: social-sharing-loop, listInbox projection: discloses sender/content/timestamp/reaction for every delivered share; unread counts unread rows
/**
 * Property-based test for the reworked inbox disclosure projection (task 2.1).
 *
 * Validates: Requirements 4.1, 6.2
 *
 * The reworked `Sharing_Service.listInbox` (design.md → "Read_State no longer
 * gates disclosure") discloses, for **every** non-deleted delivered `Share`,
 * the sender id and display name, the payload snapshot, the delivery
 * timestamp, the per-recipient `read` state, and the recipient's own
 * `Share_Reaction` — regardless of `Read_State`. `Read_State` drives only the
 * `unread` count.
 *
 * Test strategy: a `fast-check` property over a randomly generated population
 * of `share_recipients ⨝ shares ⨝ profiles ⟕ share_reactions` rows for a
 * single recipient. Each row is either `read = false` (`opened_at IS NULL`) or
 * `read = true`. The repo's `listInbox` issues exactly one `SELECT`; we drive
 * it through a fake `pg.Pool` whose `query()` returns the SQL projection the
 * repo expects:
 *
 *     { share_id, read, sender_id, sender_display_name,
 *       payload_kind, payload_snapshot, sent_at, my_reaction }
 *
 * `read` is what the production query computes via `(sr.opened_at IS NOT
 * NULL)`; the fake pool mirrors that derivation so the test exercises the
 * repo's projection logic, not a stub of it.
 *
 * The property asserts both halves of R4.1/R6.2:
 *
 *   - Every item — read or unread — carries `{ shareId, read, senderId,
 *     senderDisplayName, payloadKind, payload, sentAt, myReaction }` populated
 *     from the underlying rows (R4.1, R6.2). Disclosure never depends on
 *     `read`.
 *   - The `unread` count equals the number of rows whose `read` is `false`
 *     (R6.2).
 *
 * `numRuns: 100` per the spec convention.
 */

import { describe, it, expect } from 'vitest';
import fc from 'fast-check';

import type { SharePayload, SharePayloadKind } from '@dwt/shared';

import type { DbPool } from '../../../db/pool.js';
import { createSharingRepo } from '../repo.js';

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------

const NUM_RUNS = 100;
const RECIPIENT_ID = '11111111-1111-4111-8111-111111111111';

// The full set of fields the projection discloses for every delivered share,
// regardless of read state (R4.1, R6.2).
const DISCLOSED_KEYS = [
  'shareId',
  'read',
  'senderId',
  'senderDisplayName',
  'payloadKind',
  'payload',
  'sentAt',
  'myReaction',
] as const;

// ---------------------------------------------------------------------------
// Synthetic row shape (mirrors the repo's SQL projection)
// ---------------------------------------------------------------------------

/**
 * One synthetic `share_recipients ⨝ shares ⨝ profiles ⟕ share_reactions`
 * row, modeled after the repo's actual SELECT. `read = false` corresponds to
 * `opened_at IS NULL`; `myReaction` is `null` when the recipient has not
 * reacted (the `LEFT JOIN` produced no `share_reactions` row).
 */
interface SyntheticInboxRow {
  shareId: string;
  read: boolean;
  senderId: string;
  senderDisplayName: string;
  payloadKind: SharePayloadKind;
  payload: SharePayload;
  sentAt: Date;
  myReaction: string | null;
}

// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------

/**
 * UUID-ish string. The repo does not parse share/sender ids beyond
 * pass-through, so a stable 36-character placeholder is sufficient and
 * keeps shrinking fast.
 */
const idArb: fc.Arbitrary<string> = fc
  .integer({ min: 0, max: 0xffffffff })
  .map((n) => {
    const hex = n.toString(16).padStart(8, '0');
    return `${hex}-aaaa-4aaa-8aaa-${hex}${hex}aaaa`;
  });

const experiencePayloadArb: fc.Arbitrary<SharePayload> = fc.record({
  kind: fc.constant<'experience'>('experience'),
  experienceId: idArb,
  rating: fc.option(fc.integer({ min: 1, max: 10 }), { nil: undefined }),
  note: fc.option(
    fc.string({ minLength: 1, maxLength: 50 }),
    { nil: undefined },
  ),
}) as fc.Arbitrary<SharePayload>;

const progressPayloadArb: fc.Arbitrary<SharePayload> = fc.record({
  kind: fc.constant<'progress'>('progress'),
  overallPercent: fc.float({
    min: 0,
    max: 100,
    noNaN: true,
    noDefaultInfinity: true,
  }),
  perParkPercent: fc.constant({}),
  perCategoryPercent: fc.constant({}),
}) as fc.Arbitrary<SharePayload>;

const payloadArb: fc.Arbitrary<SharePayload> = fc.oneof(
  experiencePayloadArb,
  progressPayloadArb,
);

/**
 * `Date` arbitrary in a bounded range so shrinking lands on simple
 * timestamps. Dates are produced as actual `Date` instances to match
 * what the `pg` driver returns for `TIMESTAMPTZ` columns by default —
 * the repo's `toIsoTimestamp` helper expects this shape.
 */
const dateArb: fc.Arbitrary<Date> = fc
  .integer({ min: 0, max: 1_000_000_000_000 })
  .map((ms) => new Date(ms));

/** A recipient reaction, or `null` when they have not reacted. */
const reactionArb: fc.Arbitrary<string | null> = fc.option(
  fc.constantFrom('like', 'love', 'been_there', 'want_to_go'),
  { nil: null },
);

/** A single synthetic inbox row with random read/reaction state. */
const rowArb: fc.Arbitrary<SyntheticInboxRow> = fc.record({
  shareId: idArb,
  read: fc.boolean(),
  senderId: idArb,
  senderDisplayName: fc.string({ minLength: 1, maxLength: 50 }),
  payloadKind: fc.constantFrom<SharePayloadKind>('experience', 'progress'),
  payload: payloadArb,
  sentAt: dateArb,
  myReaction: reactionArb,
}).map((r) => {
  // Make payloadKind agree with payload.kind so the row is internally
  // consistent (the repo would never surface a (kind=X, payload.kind=Y)
  // mix because the INSERT writes both from the same input).
  return { ...r, payloadKind: r.payload.kind };
});

/**
 * A population of inbox rows. Empty lists are allowed so the
 * `unread === 0` empty-inbox edge is also exercised.
 */
const populationArb: fc.Arbitrary<SyntheticInboxRow[]> = fc.array(rowArb, {
  minLength: 0,
  maxLength: 30,
});

// ---------------------------------------------------------------------------
// Fake pool
// ---------------------------------------------------------------------------

/**
 * Build a fake `DbPool` whose only behavior is: when `query()` is
 * called with the inbox SELECT, return the supplied population mapped
 * into the row shape the repo expects. Any other SQL is an error so a
 * future change to `listInbox` that issues additional statements will
 * surface as a clear test failure rather than silently returning empty
 * results.
 */
function makeFakePool(rows: ReadonlyArray<SyntheticInboxRow>): DbPool {
  const fake = {
    async query(text: string, params: ReadonlyArray<unknown> = []) {
      if (
        !text.includes('FROM share_recipients sr') ||
        !text.includes('JOIN shares s') ||
        !text.includes('JOIN profiles p') ||
        !text.includes('LEFT JOIN share_reactions')
      ) {
        throw new Error(`unexpected SQL in inbox property test: ${text}`);
      }
      // The repo scopes the query by the recipient id; mirror that here
      // so the test would catch a regression that drops the predicate.
      if (params[0] !== RECIPIENT_ID) {
        throw new Error(
          `unexpected recipient param: ${String(params[0])}`,
        );
      }
      return {
        rows: rows.map((r) => ({
          share_id: r.shareId,
          read: r.read,
          sender_id: r.senderId,
          sender_display_name: r.senderDisplayName,
          payload_kind: r.payloadKind,
          payload_snapshot: r.payload,
          sent_at: r.sentAt,
          my_reaction: r.myReaction,
        })),
      };
    },
    async connect() {
      throw new Error('connect() is not used by listInbox');
    },
  };
  return fake as unknown as DbPool;
}

// ---------------------------------------------------------------------------
// Property
// ---------------------------------------------------------------------------

describe('listInbox projection: discloses every delivered share; unread counts unread rows', () => {
  it(
    'reveals sender/content/timestamp/reaction for every item; unread equals unread-row count',
    async () => {
      await fc.assert(
        fc.asyncProperty(populationArb, async (population) => {
          const repo = createSharingRepo(makeFakePool(population));

          const inbox = await repo.listInbox(RECIPIENT_ID);

          // 1) `unread` equals the count of rows whose `read` is false
          //    (R6.2 — Read_State drives only the unread count).
          const expectedUnread = population.filter((r) => !r.read).length;
          expect(inbox.unread).toBe(expectedUnread);

          // 2) The repo preserves cardinality and the input ordering of
          //    rows (the SQL ORDER BY is exercised by the repo's unit
          //    suite; here we only need a stable pairing so we can
          //    compare item-by-item).
          expect(inbox.items).toHaveLength(population.length);

          for (let i = 0; i < population.length; i += 1) {
            const row = population[i]!;
            const item = inbox.items[i]!;

            // R4.1/R6.2: disclosure never depends on `read` — every item
            // carries the full projection.
            expect(item).toEqual({
              shareId: row.shareId,
              read: row.read,
              senderId: row.senderId,
              senderDisplayName: row.senderDisplayName,
              payloadKind: row.payloadKind,
              payload: row.payload,
              sentAt: row.sentAt.toISOString(),
              myReaction: row.myReaction,
            });

            // Defensive: exactly the disclosed keys, no more, no less.
            expect(Object.keys(item).sort()).toEqual(
              [...DISCLOSED_KEYS].sort(),
            );
          }
        }),
        { numRuns: NUM_RUNS },
      );
    },
  );
});
