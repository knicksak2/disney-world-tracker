...
25. WHEN a User enters a search query containing at least 1 non-whitespace character, THE Catalog_Service SHALL match Experiences whose normalized name satisfies union matching: (1) normalized name contains normalized query as a contiguous substring, OR (2) every query token matches as a prefix of a token in the normalized Experience name, OR (3) every query token matches as a prefix of a token across Experience metadata (`land` including generic trailing "land" compound splitting, `world_showcase_country`, `sub_type`, `grouped_facets` display names, `interest_facets` display names, `physical_considerations` display names, `height_requirement` display name, and `accessibility` tags). This requirement extends Requirement 1.20 by superset. Normalization SHALL map ampersands/`and`/`'n'`/`n` to ` and `, strip quotes and apostrophes (including modifier letters), fold diacritics/accents via NFD decomposition, and replace punctuation with spaces.
26. THE search matching SHALL guarantee a strict superset over exact substring matches: every Experience matching `name.includes(normalizedQuery)` MUST be returned and MUST rank strictly above any metadata-only fallback match.
27. Search results SHALL be ordered by flat relevance tier score descending: Exact Name Match (100) > Name Starts With Query (80) > Contiguous Phrase in Name (60) > All Query Tokens Prefix-Matched in Name (40) > Metadata Fallback Match (20), tie-breaking deterministically by `lower(name)` ascending and `id` ascending.
28. Metadata-only matches (score 20) SHALL be ranked after all name matches (scores >= 40), sorted by `lower(name)` ascending then `id` ascending, and capped at a maximum of `MAX_METADATA_FALLBACK_ROWS` (25) entries per query.
