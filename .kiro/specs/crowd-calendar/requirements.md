...
5. WHEN recording a sample, THE System SHALL update the affected Ride_Shape and season-resolved bucket for that `(day_of_week, hour)` via a per-pass recency-weighted (EMA / capped-count) update so recent observations dominate. THE **day's Crowd_Index is a per-day aggregate**, NOT a per-slice EMA: it SHALL be derived from that day's operating-hour samples (a running daily average over core hours, normalized to the ratio scale of R2.1), finalized once the day's samples exist. THE System SHALL record a wait value only for an Experience that is operating with a posted standby wait; closed, down, or no-wait observations SHALL NOT be recorded as wait values (they MAY inform downtime metadata).
