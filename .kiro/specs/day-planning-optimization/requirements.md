...
4. THE optimizer SHALL compute travel time from the Experiences' geographic coordinates using Haversine distance with a 1.4× walking-path factor; WHERE an Experience has no coordinates, it SHALL apply a default intra-park travel time. Standalone unlocated breaks (null park/coords) SHALL be travel-neutral (skipped in the travel chain, routing travel directly between adjacent located items).
...
14. THE optimizer SHALL model queue wait as `0` for dining items (`category = 'Restaurant'`) and break items (`item_type = 'break'`), incurring cost solely from duration. Duration precedence SHALL strictly be: (1) `item.durationMinutes` (user override) if non-null; (2) `DEFAULT_BREAK_DUR` (60 min) for breaks; (3) `sub_type` defaults (Quick Service: 30 min, Table Service: 60 min, Signature Dining: 90 min, unknown: 60 min) for dining; (4) `catalogDurationMinutes ?? DEFAULT_SHOW_DURATION_MIN` (30 min) for shows and parades; (5) `DEFAULT_RIDE_DUR` (15 min) for rides/attractions.
...
17. THE Schedule Builder Item Settings modal SHALL surface an informational warning when a chosen `meal_period` is not present in the restaurant's `servedMealPeriods` (case-insensitive match against `experiences.meal_periods`), without blocking selection.
...
2. THE Schedule Builder SHALL provide an inline **"+ Add to [Date]"** action opening a tabbed `ExperiencePicker` modal (Rides, Shows, Dining, Break), allowing users to browse/search catalog attractions, shows, dining spots, or create standalone unlocated breaks with custom titles and duration chips directly on the active date without leaving the Schedule Builder.
...
4. THE Schedule Builder SHALL provide an Item Settings control with a 3-state Timing Mode control (**Any time** / **Around… [soft window]** / **At… [exact time]**) using the **Native Time Wheel Picker Dialog**, allowing users to pick exact times (`is_fixed = true`), select preset meal preference windows (`MEAL_WINDOWS`), full service windows (`MEAL_SERVICE_WINDOWS`), time-of-day presets (Morning, Midday, Afternoon, Evening), or custom time ranges (`window_start_minutes`/`window_end_minutes`), toggle `is_lightning_lane` (⚡ with pass time), toggle `use_single_rider` (👤), set `priority` (1-3), and set custom durations. The modal SHALL branch on `category` and `item_type` and SHALL NOT render a generic "Attraction vs Dining / Break" toggle.
