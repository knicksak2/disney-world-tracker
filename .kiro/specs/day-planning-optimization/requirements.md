...
9. THE Schedule Builder Item Settings modal SHALL NOT persist an explicit `durationMinutes` that the user did not explicitly choose (only sending `durationMinutes` in the edit payload when the user actively selected a duration chip), allowing unspecified durations to derive naturally from category/sub_type defaults.
