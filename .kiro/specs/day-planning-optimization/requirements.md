...
3. THE Schedule Builder SHALL render a persistent chronological **Itinerary Timeline** for the selected day, displaying items assigned to that day along with arrival times, predicted wait times for attractions and rides (omitting wait pills for breaks, dining, and non-queue items), dining/LL/show badges, and walking time connectors (`+3m walk`) derived from attraction coordinates.
...
11. WHEN a planned item has a `custom_title` and an associated location experience (`experience_name` / `experience_id`), THE Schedule Builder timeline attraction card, unscheduled item card, and item settings modal SHALL display the location name (e.g. `📍 <Location Name>`), and travel legs to that item SHALL display transit to that destination (`Transit to <Destination>`) rather than an empty park name.
12. THE ExperiencePicker SHALL provide a Destination/Park filter chip bar (All, Magic Kingdom, EPCOT, Hollywood Studios, Animal Kingdom, Typhoon Lagoon, Blizzard Beach, Disney Springs, Resorts). WHEN a Destination/Park is selected, THE picker SHALL issue the appropriate category/parkId/areaType query and display results organized by Land using the shared pavilion-aware grouping (`groupByPavilionFiltered`) with visible section headers. Free-text search SHALL combine conjunctively with active chips.
13. WHEN the Schedule Builder opens the ExperiencePicker for a date whose Day Touring Hours specify a startingPark, THE picker SHALL pre-select that Park's filter chip; otherwise it SHALL default to All. THE picker SHALL always expose an All chip so the pre-selection can be cleared.
14. On the Breaks tab, location searching SHALL be an unrestricted location search across the catalog scoped to the selected park (not restricted to Resort category), and SHALL require at least 2 characters of search text (`debouncedQuery.length >= 2`) before issuing a query or rendering location results so the break composer is not flooded before typing.
15. THE ExperiencePicker SHALL provide a compact Multi-Select Filter Row featuring top high-signal Quick-Chips (top 3–4 popular category tags including dining service styles and price tiers), a `[ ⚙️ Filters (N) ]` action button, and a `[ ✕ Reset ]` chip when filters are active. Tapping `[ ⚙️ Filters ]` SHALL open a structured Filters Bottom Sheet modal containing dedicated, scrollable sections for Lands, Price Range (`$`, `$$`, `$$$`, `$$$$`), and Attributes & Dining tags. Attribute and price chips SHALL be derived from non-null `priceTier` and high-signal facet groups (`interests`, `thrillFactor`, `parkInterests`, `disneyFavorites`, `tableService`, `quickService`, `dining`, and `subType`), explicitly excluding noisy `age` and `height` groups, and deduping chips by `id` OR case-insensitive trimmed `name`. Users SHALL be able to select multiple chips simultaneously, where selections combine disjunctively (`OR`) within the same dimension (multiple Lands, multiple Price Tiers, or multiple Attributes) and conjunctively (`AND`) across dimensions (selected Lands AND selected Price/Attributes). Each filter chip in the row and modal SHALL expose full accessibility metadata (`accessibilityRole="checkbox"`, `accessibilityState.checked`, and descriptive `accessibilityLabel`). WHEN `activeTab` or `selectedPark` changes, or WHEN the user taps the Reset chip or modal Clear All action, all active filter selections SHALL be cleared.
