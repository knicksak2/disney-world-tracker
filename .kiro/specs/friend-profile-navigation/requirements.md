...
3. THE App SHALL provide, for the Filter_Park_Selection, a dropdown selector control displaying the active Park selection and opening a selection picker offering "All" plus exactly one option for each Park defined in the catalog, and SHALL provide, for the Filter_Category_Selection, a dropdown selector control displaying the active Category selection and opening a selection picker offering "All" plus exactly one option for each Experience_Category defined in the catalog.
...
10. THE App SHALL provide a search input within the Experience_Filter that filters loaded Completion_Entries by matching a case-insensitive query against experience name, park, category, or note, without re-fetching data.
11. WHEN any non-default selection or search query is active, THE App SHALL provide a reset control that restores the filter to a Filter_Park_Selection of "All", a Filter_Category_Selection of "All", and an empty search query in a single interaction.
